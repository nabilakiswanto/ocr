...
const WebcamCapture = () => {
  ...
  const handleCapture = async () => {
    ...
    try {
      // Use the raw image directly without preprocessing
  ...
  const parseText = (text) => {
    console.log("OCR Text:", text); // Debug: Log the raw OCR text
  ...
  // if (isMobile) {
    return (
      <div style={{ textAlign: 'center' }}>
        ...
        <button onClick={handleCapture} disabled={loading} className="mt-5">
